...
class InstallCommand extends Command{
    ...
    public function handle(Filesystem $filesystem){
        ...
        if(false === strpos($routes_contents, 'Voyager::routes()')){
            $filesystem->append(base_path('routes/web.php'), PHP_EOL . PHP_EOL . "Route::group(['prefix' => 'admin'], function () {" . PHP_EOL . "    Voyager::routes();" . PHP_EOL . "});" . PHP_EOL);
        }
        ...
        if($this->option('with-dummy')){
            ...
            $this->addNamespaceIfNeeded(collect($filesystem->files("{$publishablePath}/database/dummy_seeds/")), $filesystem);
        } else {
            $this->call('vendor:publish', [
                '--provider' => VoyagerServiceProvider::class,
                '--tag' => ['config', 'voyager_avatar'],
            ]);
        }

        $this->addNamespaceIfNeeded(collect($filesystem->files("{$publishablePath}/database/seeds/")), $filesystem);
    ...
    private function addNamespaceIfNeeded($seeds, Filesystem $filesystem){
        ...
        $seeds->each(function($file) use ($filesystem){
            ...
            ], "<?php" . PHP_EOL . PHP_EOL . "namespace Database\\Seeders;" . PHP_EOL . PHP_EOL . "use", $filesystem->get($path));
